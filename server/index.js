...
async function handlePaintParent (ws, msg) {
  ...
  if (!prevPixel) {
    const isProt = await redis.isProtectedTile(tileKey, space)
    if (isProt) {
      ws.send(JSON.stringify({ type: CONSTANTS.WS_TYPE_PAINT_ERROR, id: msg.id, reason: 'protected' }))
      return
    }
...
async function handlePaintChild (ws, msg) {
  ...
  if (!parentPixel) {
    const isProt = await redis.isProtectedTile(parentId, space)
    if (isProt) {
      ws.send(JSON.stringify({ type: CONSTANTS.WS_TYPE_PAINT_ERROR, id: msg.id, reason: 'protected' }))
      return
    }
...
async function handleRequest (req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/protected-regions') {
      setCORS(res)
      try {
        const space = parseSpaceParam(url)
        const regions = await redis.getProtectedRegions(space)
        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify(regions))
      } catch (err) {
        console.error('GET /protected-regions error:', err)
        sendError(res, 500, 'Internal server error')
      }
      return
    }
    ...
    if (req.method === 'POST' && url.pathname === '/admin/protect') {
      ...
      try {
        ...
        const result = await redis.protectRegion(n, s, e, w, space)
        ...
        broadcastToSpace(space, { type: CONSTANTS.WS_TYPE_REGIONS_CHANGED })

        res.writeHead(200, { 'Content-Type': 'application/json' })
        res.end(JSON.stringify({ ok: true, regionId: result.regionId, tilesCount: result.tilesCount, protected: result.protected }))
      } catch (err) {
        if (err.message === 'Body too large') { sendTooLarge(res); return }
    ...
    if (req.method === 'POST' && url.pathname === '/admin/unprotect') {
      ...
      try {
        ...
        await redis.unprotectRegion(data.regionId, space)
...
function broadcastToSpace (space, data) {
  const msg = JSON.stringify(data)
  for (const client of wss.clients) {
    if (client.readyState === CONSTANTS.WS_OPEN && client.space === space) {
      client.send(msg)
    }
  }
}
